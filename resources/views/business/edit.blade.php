...
@section('content')
            ...
            <form id="DataForm" action="{{ route('profile.update') }}" method="POST">
                <div class="form-row mb-3">
                  <div class="col">
                    ...
                    <input type="text" class="form-control" placeholder="Nama Usaha" name="name" value="{{ $business->name }}" required>
                  </div>
                  <div class="col">
                    ...
                    <select class="form-control" id="BusinessType" name="type">
                        ...
                        <option name="DISTRIBUTOR" {{ $business->role == "DISTRIBUTOR" ? "selected" : "" }}>DISTRIBUTOR</option>
                ...
                <a type="button" class="btn btn-secondary float-right mr-2" href="{{ route('business.index') }}"><i class="fas fa-chevron-left mr-2"></i>Kembali</a>
            </form>
        </div>
    </div>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <button class="btn btn-success mb-3" data-toggle="modal" data-target="#AddOffice"><i class="fas fa-plus mr-2"></i> Tambah Cabang</button>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <button class="btn btn-success mb-3" data-toggle="modal" data-target="#AddEmployee"><i class="fas fa-plus mr-2"></i> Tambah Pegawai</button>
            <div class="table-responsive">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <button class="btn btn-success mb-3" data-toggle="modal" data-target="#AddScanner"><i class="fas fa-plus mr-2"></i> Tambah Perangkat</button>
            <div class="table-responsive">
    ...
    {{-- Scanner --}}
    <div class="modal fade" id="AddScanner" tabindex="-1" role="dialog" aria-labelledby="AddScannerLabel" aria-hidden="true">
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="AddScannerForm" action="{{ route('business.store_scanner') }}" method="POST">
                    ...
                    <div class="form-group">
                        <input type="text" class="form-control" placeholder="Nama Perangkat" name="name" required>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
              <button type="submit" class="btn btn-success" form="AddScannerForm"><i class="fas fa-save mr-2"></i>Simpan</button>
            </div>
          </div>
        </div>
    </div>
    <div class="modal fade" id="EditScanner" tabindex="-1" role="dialog" aria-labelledby="EditScannerLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="EditScannerLabel">Ubah Perangkat</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Tutup">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
                <form id="EditScannerForm">
                    <div class="form-group">
                        <select class="form-control" name="office">
                            @foreach ($office as $item)
                                <option value="{{ $item->slug }}">{{ $item->name }}</option>
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
              <button type="submit" class="btn btn-success" form="EditScannerForm"><i class="fas fa-save mr-2"></i>Simpan</button>
            </div>
          </div>
        </div>
    </div>
